...
class AdminController {
    ...
    public function updateStatus(string $id): void {
        ...
        $pdo->prepare('INSERT INTO repair_status_history (repair_id,status_id,changed_by,note) VALUES (?,?,?,?)')
            ->execute([(int)$id, $status_id, $_SESSION['user_id'], $note ?: 'Status zmieniony przez admina']);
    ...
    public function sendQuote(string $id): void {
        ...
        $pdo->prepare('UPDATE repairs SET status_id=?, updated_at=NOW() WHERE id=?')
            ->execute([$status_id, (int)$id]);
        $pdo->prepare('INSERT INTO repair_status_history (repair_id,status_id,changed_by,note) VALUES (?,?,?,?)')
            ->execute([(int)$id, $status_id, $_SESSION['user_id'], $hist_note . ($note ? '. ' . $note : '')]);
    ...
    public function markPaid(string $id): void {
        ...
        // Dodaj płatność
        $pdo->prepare('INSERT INTO payments (repair_id,amount,method,status,paid_at) VALUES (?,?,?,\'paid\',NOW())')
            ->execute([(int)$id, $repair['final_quote_amount'], $method]);
        ...
        $pdo->prepare('UPDATE repairs SET status_id=?, updated_at=NOW() WHERE id=?')
            ->execute([$status_id, (int)$id]);
        $pdo->prepare('INSERT INTO repair_status_history (repair_id,status_id,changed_by,note) VALUES (?,?,?,?)')
            ->execute([(int)$id, $status_id, $_SESSION['user_id'], 'Płatność potwierdzona. Metoda: ' . $method]);
    ...
    public function markReturning(string $id): void {
        requireAdmin();
        global $pdo;
        $note = trim($_POST['note'] ?? 'Klient zrezygnował z naprawy - zwrot sprzętu');
        ...
        redirect('/admin/naprawa/' . $id . '?success=Zgłoszenie oznaczone jako zwrot sprzętu');
    }
